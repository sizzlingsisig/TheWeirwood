<x-layouts.app>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        ...
        <div class="bg-gradient-to-br from-[rgba(61,43,31,0.5)] to-[rgba(26,21,18,0.8)] border border-[rgba(184,134,11,0.2)] rounded-sm p-6 shadow-lg">
            <div class="flex items-center justify-between mb-3">
                <div>
                    <p class="font-['Cinzel'] text-[0.65rem] font-bold tracking-[0.2em] text-[#6B5A4E] uppercase mb-1">Houses Unlocked</p>
                    <p class="font-['Cinzel'] text-3xl font-bold text-[#E8DCC8] drop-shadow-md">{{ $unlockedHouses ?? 0 }} / {{ $totalHouses ?? 0 }}</p>
                </div>
                <div class="text-[#B8860B] opacity-80">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="h-2 bg-[rgba(26,21,18,0.8)] rounded-full overflow-hidden">
                <div class="h-full bg-gradient-to-r from-[#8B0000] to-[#B8860B] transition-all duration-500" style="width: {{ $totalHouses ? (($unlockedHouses ?? 0) / $totalHouses) * 100 : 0 }}%"></div>
            </div>
            <p class="font-['Cinzel'] text-xs text-[#6B5A4E] mt-2 text-right">{{ $totalHouses ? round((($unlockedHouses ?? 0) / $totalHouses) * 100) : 0 }}%</p>
        </div>
        ...
        <div class="bg-gradient-to-br from-[rgba(61,43,31,0.5)] to-[rgba(26,21,18,0.8)] border border-[rgba(184,134,11,0.2)] rounded-sm p-6 shadow-lg">
            <div class="flex items-center justify-between mb-3">
                <div>
                    ...
                    <p class="font-['Cinzel'] text-3xl font-bold text-[#E8DCC8] drop-shadow-md">{{ $discoveredEndings ?? 0 }} / {{ $totalEndings ?? 0 }}</p>
                </div>
                <div class="text-[#B8860B] opacity-80">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="h-2 bg-[rgba(26,21,18,0.8)] rounded-full overflow-hidden">
                <div class="h-full bg-gradient-to-r from-[#8B0000] to-[#B8860B] transition-all duration-500" style="width: {{ $totalEndings ? (($discoveredEndings ?? 0) / $totalEndings) * 100 : 0 }}%"></div>
            </div>
            <p class="font-['Cinzel'] text-xs text-[#6B5A4E] mt-2 text-right">{{ $totalEndings ? round((($discoveredEndings ?? 0) / $totalEndings) * 100) : 0 }}%</p>
        </div>
    </div>
